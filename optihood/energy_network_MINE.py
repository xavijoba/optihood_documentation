...
class EnergyNetworkClass(solph.EnergySystem):
    ...
    def optimize(self, numberOfBuildings, solver, envImpactlimit=1000000, clusterSize={},
        ...
        if solver == "gurobi" or solver =="cbc":
    ...
    def _postprocessingClusters(self, clusterSize):
        # self._optimizationResults["electricalBus"] = mydict.pop("electricalBus__Building1")
    ...
    def _calculateResultsPerBuilding(self, mergeLinkBuses):
        for b in self.__buildings:
            ...
            #errore dovuto a mancanza di label nei risultati
            if ((mergeLinkBuses and buildingLabel=='Building1') or not mergeLinkBuses) and \
                ...
                self.__feedIn[buildingLabel] = sum(solph.views.node(self._optimizationResults, 
                                                                    electricityBusLabel)["sequences"][
                                                                        (electricityBusLabel, 
                                                                         excessElectricityBusLabel), "flow"]) * self.__costParam[excessElectricityBusLabel]
